...
func NewRTCToken(appID, appKey, roomID, userID string) *Token {
	return &Token{
		AppID:      appID,
		AppKey:     appKey,
		RoomID:     roomID,
...
func Parse(raw string) (*Token, error) {
	...
	if raw[:VersionLength] != RTCVersion {
		return nil, fmt.Errorf("expect RTCVersion: %s, got %s", RTCVersion, raw[:VersionLength])
	}
...
func (t *Token) Serialize() (string, error) {
	...
	return RTCVersion + t.AppID + base64.StdEncoding.EncodeToString(bytesContent), nil
}
